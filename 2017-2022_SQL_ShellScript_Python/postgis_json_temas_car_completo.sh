...
ogr2ogr -f "GeoJSON"  pessoal_br_nomes_20180828.json PG:"host=172.16.32.248 user=postgres dbname=car_nacional201808"  -sql "SELECT  municipio.cod_estado,  imovel_pessoa.idt_imovel,  imovel_pessoa.ind_tipo_pessoa,  imovel_pessoa.cod_cpf_cnpj,  imovel_pessoa.nom_completo,  imovel.ind_status_imovel,  imovel.ind_tipo_imovel,  documento_imovel.idt_documento_imovel,  tipo_documento.cod_tipo_documento FROM  usr_geocar_aplicacao.imovel,  usr_geocar_aplicacao.municipio,  usr_geocar_aplicacao.imovel_pessoa,  usr_geocar_aplicacao.tipo_documento,  usr_geocar_aplicacao.documento_imovel, ST_centroid(imovel.geo_area_imovel) as geom WHERE  municipio.idt_municipio = imovel.idt_municipio AND imovel_pessoa.idt_imovel = imovel.idt_imovel AND tipo_documento.idt_tipo_documento = documento_imovel.idt_tipo_documento AND documento_imovel.idt_imovel = imovel.idt_imovel  order by municipio.cod_estado, cod_tipo_documento" -nln nomes -nlt point

ogr2ogr -f "GeoJSON" -update -append  publico_br_imoveis_20180822.json PG:"host=172.16.32.248 user=postgres dbname=car_nacional201808"  -sql "SELECT    imovel.idt_imovel, imovel.cod_imovel, imovel.cod_protocolo, imovel.dat_protocolo, imovel.ind_status_imovel, imovel.ind_tipo_imovel, imovel.dat_criacao, imovel.dat_atualizacao,  imovel.num_area_imovel as area_imovel,   imovel.num_modulo_fiscal as num_mf, municipio.idt_municipio as geocodigo,    municipio.cod_estado,    rel_tema_imovel_poligono.the_geom as geom FROM    usr_geocar_aplicacao.imovel,    usr_geocar_aplicacao.municipio,    usr_geocar_aplicacao.rel_tema_imovel_poligono,    usr_geocar_aplicacao.tema WHERE  tema.idt_tema = 26 AND   municipio.idt_municipio = imovel.idt_municipio AND   rel_tema_imovel_poligono.idt_imovel = imovel.idt_imovel AND   tema.idt_tema = rel_tema_imovel_poligono.idt_tema order by municipio.cod_estado" -nln imoveis -nlt MULTIPOLYGON
